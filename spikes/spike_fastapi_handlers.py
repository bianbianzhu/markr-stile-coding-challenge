...
async def h_starlette(_, exc: StarletteHTTPException):
    if exc.status_code == 404:
        return JSONResponse(
            status_code=404,
            content={"error": "not_found", "message": "x", "details": {"reason": "unknown_route"}},
        )
    if exc.status_code == 405:
        return JSONResponse(
            status_code=405, content={"error": "method_not_allowed", "message": "x", "details": {}}
        )
    return JSONResponse(
        status_code=exc.status_code,
        content={"error": "internal_error", "message": "x", "details": {}},
    )
...
async def h_validation(_, exc: RequestValidationError):
    return JSONResponse(
        status_code=422,
        content={"error": "invalid_path_param", "message": "x", "details": {"raw": str(exc)}},
    )
...
@app.get("/markr")
async def m():
    raise MarkrHTTPException(422, "wrong_root", "x")
...
@app.get("/p/{x}")
async def p(x: Annotated[str, Path(max_length=3)]):
    return {"x": x}
